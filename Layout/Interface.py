...
def Interface():
    ...
    equation = ttk.Entry(frame_init, width=14)
    equation.grid(column=1, row=1, sticky=(tk.W, tk.E), pady=1)
    equation.insert(0, "6 * ln(0.1+Abs(x)*3) + cos(x*2)")
    ...
    button = ttk.Button( text="Minimizacion",
                        command=minimizar)
    ...
    button = ttk.Button( text="Maximizacion",
                        command=maximizar)
    button.grid(column=1, row=4, sticky=tk.W, padx=5, pady=5)
